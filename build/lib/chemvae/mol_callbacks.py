"""Callbacks for training VAEs and property prediction models."""
...
class RmseCallback(Callback):
    """Callback to calculate RMSE and MAE on test set after each epoch."""
    ...
    def __init__(self, X_test, Y_test, params, df_norm: pd.DataFrame = None):
        ...
        :param df_norm: dataframe containing mean and std of the property
        """
    ...
    def on_epoch_end(self, epoch, logs=None):
        """
        Calculate RMSE and MAE on test set after each epoch.
...
class WeightAnnealer_epoch(Callback):
    ...
    :param schedule: a function that takes an epoch index as input (integer, indexed from 0) and returns a new weight for the VAE (float). Currently just adjust kl weight, will keep xent weight constant
    """
    ...
    def __init__(self, schedule, weight, weight_orig, weight_name):
        ...
        :param schedule: a function that takes an epoch index as input (integer, indexed from 0) and returns a new weight for the VAE (float). Currently just adjust kl weight, will keep xent weight constant
        :param weight: weight of the VAE
        :param weight_orig: original weight of the VAE
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        assert isinstance(new_weight, float), 'The output of the "schedule" function should be float.'
...
def sample(a, temperature=0.01):
    ...
    :param a: array of probabilities
...
class EncoderDecoderCheckpoint(ModelCheckpoint):
    """Adapted from ModelCheckpoint, but for saving Encoder, Decoder and property prediction models."""
    ...
    def __init__(
        ...
        :param decoder_model: decoder model
        :param params: dictionary of parameters
        :param prop_pred_model: property prediction model
        :param prop_to_monitor: property to monitor
        :param save_best_only: whether to save only the best model
        :param monitor_op: operation to monitor
        :param monitor_best_init: initial value of the property to monitor
        """
    ...
    def on_epoch_end(self, epoch, logs=None):
        """
        Save models at the end of every epoch if they are better than previous models.
