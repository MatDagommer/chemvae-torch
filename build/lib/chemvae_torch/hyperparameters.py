"""Get hyperparameters for training the model."""
...
def load_params(param_file=None, verbose=True):
    """
    Load parameters from json file and return as dictionary.

    :param param_file: path to json file containing parameters
    :param verbose: whether to print parameters
    :return: dictionary of parameters
    """
    # Parameters from params.json and exp.json loaded here to override parameters set below
    if param_file is not None:
        hyper_p = json.loads(open(param_file).read(), object_pairs_hook=OrderedDict)
        if verbose:
