"""Train a VAE on a dataset of SMILES strings."""
...
def vectorize_data(params):
    """
    Vectorize the data as one-hot vectors.

    :param params: dictionary of parameters
    :return: X_train, X_test, Y_train, Y_test
    :raises ValueError: if no logit tasks or regression tasks specified for property prediction
    """
    ...
    if params["do_prop_pred"] and ("data_file" in params):
        ...
        if ("reg_prop_tasks" in params) and ("logit_prop_tasks" in params):
            ...
            smiles, Y_logit = mu.load_smiles_and_data_df(
                params["data_file"], MAX_LEN, logit_tasks=params["logit_prop_tasks"], normalize_out=normalize_out
            )
        elif "reg_prop_tasks" in params:
            smiles, Y_reg = mu.load_smiles_and_data_df(
                params["data_file"], MAX_LEN, reg_tasks=params["reg_prop_tasks"], normalize_out=normalize_out
            )
        else:
    ...
    # Vectorization is a one-hot encoding of the smiles strings, with padding
    # when the number of characters per string is less than the maximum
    print("Vectorization...")
    X = mu.smiles_to_hot(smiles, MAX_LEN, params["PADDING"], CHAR_INDICES, NCHARS)

    # Crop data to keep the number of points proportional to the batch size
    ...
    # Retrieve number of training points, ensuring it is proportional to the batch size
    ...
    # Separate indices into training and test
    ...
    # Retrieve corresponding data points
    ...
    # Retrieve associated property values if property prediction is to be done
    if params["do_prop_pred"]:
    ...
    else:  # if no prop pred, just autoencoding task
        return X_train, X_test


def load_models(params):
    """
    Load the encoder, decoder and property predictor models.
    ...
    # z_mean: sampled mean values? | enc_output: output of the encoder
    z_mean, enc_output = encoder(x_in)
    # retrieving sampled points z_samp (from mean and var) and the mean and var used for sampling
    # concatenated in z_mean_log_var_output
    z_samp, z_mean_log_var_output = variational_layers(z_mean, enc_output, kl_loss_var, params)

    # Decoder
    if params["do_tgru"]:  # teacher forcing
    ...
    # Autoencoder only, no property prediction
    ...
    if params["do_prop_pred"]:
        ...
        # Autoencoder with property prediction
        # AE_PP_model trains three networks the property predictor, the encoder, and the decoder
...
def kl_loss(truth_dummy, x_mean_log_var_output):
    ...
    :param truth_dummy: dummy variable
    :param x_mean_log_var_output: output of the encoder
    :return: kl_loss
    """
...
def main_no_prop(params):
    """
    Train the VAE without property prediction.

    :param params: dictionary of parameters
    :return: None
    :raises NotImplementedError: if optimizer is not defined
    """
    ...
    vae_sig_schedule = partial(
        mol_cb.sigmoid_schedule, slope=params["anneal_sigmod_slope"], start=params["vae_annealer_start"]
    )
    ...
    model_test_targets = {"x_pred": X_test, "z_mean_log_var": np.ones((np.shape(X_test)[0], params["hidden_dim"] * 2))}
...
def main_property_run(params):
    """
    Train the VAE with property prediction.
    ...
    # Load data
    # X_train, X_test: one-hot vectorized smiles
    # Y_train, Y_test: property values
    ...
    model_loss_weights = {"x_pred": ae_loss_weight * xent_loss_weight, "z_mean_log_var": ae_loss_weight * kl_loss_var}
    ...
    if "checkpoint_path" in params.keys():
        callbacks.append(
            mol_cb.EncoderDecoderCheckpoint(
                encoder, decoder, params=params, prop_pred_model=property_predictor, save_best_only=False
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--exp_file", help="experiment file", default="exp.json")
