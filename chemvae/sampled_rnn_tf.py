"""RNN with sampling of the output at each timestep."""
...
def sampled_rnn(
    ...
    :param units : number of units in the output dimension.
    :param random_seed: random seed for sampling
    :param go_backwards: boolean. If True, do the iteration over
        the time dimension in reverse order.
    :param mask: binary tensor with shape `(samples, time, 1)`,
        with a zero for every element that is masked.
    :param rec_dp_constants: a list of constant values passed at each step.
    ...
    :return: A tuple, `(last_output, outputs, new_states)`.
    :raises ValueError: if input dimension is less than 3.
    """
    ...
    outputs, _ = step_function(
        inputs[0],
        {"initial_states": initial_states, "random_cutoff_prob": random_cutoff_prob, "rec_dp_mask": rec_dp_constants},
    )
    ...
    def _step(time, output_ta_t, *states):
        ...
        :param output_ta_t: tensor array for output
        :param states: list of states
        :return: tuple of (time + 1, output_ta_t) + tuple(new_states)
        """
        ...
        output, new_states = step_function(
            current_input,
            {"initial_states": states, "random_cutoff_prob": random_cutoff_prob, "rec_dp_mask": rec_dp_constants},
        )
